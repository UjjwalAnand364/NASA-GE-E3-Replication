...
C   plane model in 1979, soon after the author moved to Cambridge.      C
C                                                                      C
...
C   OF THE  PARAMETER STATEMENTS IN "commall-open-17.5"                C
C                                                                      C
...
C
      ...
      IF( (BETADWN2(NR,K).GT.0.0001).OR.(BETADWN2(NR,K).GT.0.0001) )
     &   IF_ANGLES(NR) = 1
C
C***********RT_THICK(J,K) IS BLADE THICKNESS DELTA R-THETA MEASURED
C           IN THE TANGENTIAL DIRECTION.
...
C
      INCLUDE 'commall-open-17.5'
C
      DIMENSION SUMAS(JD),ROVMSQ(KD),ANG_INC(KD)
      SAVE START,FINI
C
      RIJKM  = 2./(IM*JM*KM)
C
...
C
      ...
      PSLIM  = AMIN1(P(IMID,1,KMID),P(IMID,JM,KMID))
      PSLIM  = 0.5*PSLIM
      RIJKM  = 2./(IM*JM*KM)
      ROCIN  = GA*P(IMID,1,KMID)*RO(IMID,1,KMID)
      ROCOUT = GA*P(IMID,JM,KMID)*RO(IMID,JM,KMID)
      ROCLIMSQ = 0.1*AMIN1(ROCIN,ROCOUT)
      ROVM_IN  = SQRT(ROVX(IMID,1,KMID)*ROVX(IMID,1,KMID)
     &         + ROVR(IMID,1,KMID)*ROVR(IMID,1,KMID))
      ROVM_OUT = SQRT(ROVX(IMID,JM,KMID)*ROVX(IMID,JM,KMID)
     &         + ROVR(IMID,JM,KMID)*ROVR(IMID,JM,KMID))
      ROVM_REF  = 0.05*AMIN1(ROVM_IN,ROVM_OUT)
C
      DO 4001 K=1,KM
...
C
      FMACH  = 1.0 + MACHLIM*MACHLIM*0.5*(GA -1.0)
      DO 7250 NR = 1,NROWS
      JREF   = JLE(NR)
      TREF   = T_STATIC(IMID,JREF,KMID)
      VREFSQ = VX(IMID,JREF,KMID)*VX(IMID,JREF,KMID) +
     &         VR(IMID,JREF,KMID)*VR(IMID,JREF,KMID) +
     &         WT(IMID,JREF,KMID)*WT(IMID,JREF,KMID)
      TOREF  = TREF + 0.5*VREFSQ/CP
      TLIMIT = TOREF/FMACH
      VLIMIT(NR) = SQRT(2*CP*(TOREF - TLIMIT) )
 7250 CONTINUE
C
...
C
      ...
      V_LIMIT = VLIMIT(NR)
      DO 7100 I=1,IM
      ROWRELSQ    = ROVX(I,J,K)*ROVX(I,J,K) + ROVR(I,J,K)*ROVR(I,J,K)
     &            + ROWT(I,J,K)*ROWT(I,J,K)
      WREL        = SQRT(ROWRELSQ)/RO(I,J,K)
      FSAFE_1     = V_LIMIT/WREL
      ROCSQ       = GA*P(I,J,K)*RO(I,J,K)
      IF(ROCSQ.LT.ROCLIMSQ) ROCSQ = ROCLIMSQ
      MACHREL     = SQRT(ROWRELSQ/ROCSQ)
      IF(ITIMST.EQ.5.OR.ITIMST.EQ.6)
     &              MACHREL = SQRT(ROWRELSQ)/RO(I,J,K)/VSOUND
      FSAFE_2     = MACHLIM/MACHREL
      FACSAFE     = AMIN1(FSAFE_1,FSAFE_2)
C
      IF(FACSAFE.LT.1.0) THEN
           ROVX(I,J,K) = ROVX(I,J,K) * FACSAFE
           ROVR(I,J,K) = ROVR(I,J,K) * FACSAFE
           ROVTREL     = ROWT(I,J,K) * FACSAFE
           ROVT(I,J,K) = ROVTREL + RO(I,J,K)*UBLADE(J,K) 
...
C
      ...
      DO 5665 I=1,IMM1
      DO 5665 K=1,KMM1
      SUMAS(J) = SUMAS(J)  - FLOWX(I,J,K)
 5665 CONTINUE
      SUMAS(J) = (SUMAS(J) + SHRDFLOW(J))*NB + SUMBLEED(J) - SUMCWL(J)
 5675 CONTINUE
      ECONT    = 0.0
      DO 5677 J=1,JM
      NB  = NBLADE(J)
      RATIO = SUMAS(J)/SUMAS(1)
      EMASS = ABS(1.-RATIO)
...
C
      RATFLOW = SUMAS(JM)/FLOWRAT
      WRITE(6,9000) NSTEP,EMAX,IMAX,JMAX,KMAX,EAVG,ECONT,JCONT,VREF,
     &              VMAX,IVMAX,JVMAX,KVMAX,FLOWRAT,SUMAS(JM),RATFLOW
...
C 9009 FORMAT('EMAX,EAVG,ECONT',3E12.5,' FLOW',E12.5,'STEP',I5,'AT',3I5)
 9009 FORMAT(3E12.5,E12.5,I10,3I5) 
C
C******************************************************************************
C******************************************************************************
...
C
      AVGCHG = DAMP*SUMCHG/(IMM1*JMM1*KMM1)
C
      DO 1525 K=1,KMM1
      DO 1525 J=2,JM
      DO 1525 I=1,IMM1
...
C
C**********************************************************************************
C       SET INITIAL GUESS OF FLOW FIELD VELOCITIES,DENSITY*VELOCITY,ETC.
C
      DO 3300 K=1,KM
      DO 3310 I=1,IM
      VT(I,1,K) = VTIN(K)
...
C
      DO 3320 J=2,JM
      AREA = SQRT(AQX(IMID,J,KMID)*AQX(IMID,J,KMID)+AQR(IMID,J,KMID)
     &           *AQR(IMID,J,KMID))*NBLADE(J)
      IF(J.EQ.2) AIN = AREA
      VS = VM1(K)*AIN*RO(1,1,1)/(AREA*RO(1,J,1))
      VX(I,J,K) = VS*DX(J,K)/DS(J,K)
      WT(I,J,K) = VS*R(J,K)*(THETA(I,J,K) - THETA(I,J-1,K))/DS(J,K)
      VT(I,J,K) = WT(I,J,K) + UBLADE(J,K)
      IF(INDMIX(J-1).EQ.1) VT(I,J,K)=VT(I,J-1,K)
      VR(I,J,K) = VS*DR(J,K)/DS(J,K)
      HO(I,J,K) = HO(I,J-1,K) + WRAD(J)*(R(J,K)*VT(I,J,K)
     &          - R(J-1,K)*VT(I,J-1,K) )
 3320 CONTINUE
      VR(I,1,K) = VR(I,2,K)
      VX(I,1,K) = VX(I,2,K)
 ...
 3300 CONTINUE
C
C    JDD MOD  29/8/2007 TO MAKE THE INITIAL GUESS OF VT CONTINUOUS IN UNBLADED REGIONS.
C
      DO 3333 K=1,KM
      DO 3333 J=1,JM
         NRW  = NROW(J) 
         IF(J.LT.JLE(1))      RVTFREE = VTIN(K)*R(1,K)
         IF(J.EQ.JTE(NRW))    RVTFREE = VT(IMID,J,K)*R(J,K)
      DO 3333 I=1,IM
         IF(IND(J).EQ.0.AND.J.GT.JTE(1) ) THEN
               VT(I,J,K) = RVTFREE/R(J,K)
...
C******************************************************************************
C 
      SUBROUTINE SUMFLX(SUMAS,SUMPO,SUMTO,SUMRVT,SUM_ENTPY,SUMTSTAT,
     &                  SUMPSTAT) 
C
...
C
      INCLUDE  'commall-open-17.5'
C
      DIMENSION SUMAS(JD),SUM_ENTPY(JD),SUMPO(JD),SUMTO(JD),SUMRVT(JD),
     &          SUMTSTAT(JD),SUMPSTAT(JD)
C
      DO 100 J=1,JM
      SUMAS(J)      = 0.0
      SUMPO(J)      = 0.0
      ...
      DO 110 K=1,KMM1
      DO 120 I=1,IMM1
      DFLOW        = -FLOWX(I,J,K)*NBLADE(J)/4
      SUMAS(J)     = SUMAS(J) + DFLOW*4
      SUMPO(J)     = SUMPO(J) + DFLOW*(TEMP2(I,J,K)+TEMP2(I+1,J,K)
     &             + TEMP2(I+1,J,K+1)+TEMP2(I,J,K+1))
      SUMTO(J)     = SUMTO(J) + DFLOW*(TEMP1(I,J,K)+TEMP1(I+1,J,K)
     &             + TEMP1(I+1,J,K+1)+TEMP1(I,J,K+1))
      SUMRVT(J)    = SUMRVT(J)+ DFLOW*(TEMP3(I,J,K)+TEMP3(I+1,J,K)
     &             + TEMP3(I+1,J,K+1)+TEMP3(I,J,K+1))
      SUM_ENTPY(J) = SUM_ENTPY(J) + DFLOW*(TEMP4(I,J,K)+TEMP4(I+1,J,K)
     &             + TEMP4(I+1,J,K+1)+TEMP4(I,J,K+1))
      SUMTSTAT(J)  = SUMTSTAT(J) + DFLOW*(STORE2(I,J,K)+STORE2(I+1,J,K)
     &             + STORE2(I+1,J,K+1)+STORE2(I,J,K+1))
      SUMPSTAT(J)  = SUMPSTAT(J)  + DFLOW*(P(I,J,K)+P(I+1,J,K)
...
C
      ...
      SNEW(J) = SNEW(J-1) + XD
C	  
C     EDITED BY GSINGH 10/10/2017	  
C
C	  WRITE(6,*) SNEW(J),(J2-1-J+1)
      IF(RAT.GT.1.) XD      = XD*RAT
	  IF(RAT.LE.1.) XD      = R1*RAT**(J2-1-J+1)
C	  END OF EDIT
C
   10 CONTINUE
C
...
C	  WRITE(6,*) 'NPUP= ',NPUP,' RUP= ',RUP,' RAT= ',RAT
C	  WRITE(6,*) ' XUP= ',XUP,' DX1= ',DX1
      M = 1 
  50  RHS = (RAT-1.)*RUP 
...
C
      REAL MACHCOOL
C
      WRITE(6,*) ' ENTERED COOLIN'
C
C
C    READ IN THE COOLING FLOW DETAILS FOR THE CURRENT BLADE ROW.
C
      NCOOLB     = 0
      NCOOLW     = 0
      NCWLBLADEP = 0
...
C
      DIMENSION SUMAS(JD),SUM_ENTPY(JD),SUMPO(JD),SUMTO(JD),SUMRVT(JD),
     &          SUMTSTAT(JD), ETA_LOSS(JD),SUMPSTAT(JD)
C
...
C      AND STATIC TEMPERATURE. FOR USE IN SUBROUTINR SUMFLX.
C
C    TEMP3  stores RVT
...
C
 5610 CONTINUE
C
      CALL SUMFLX(SUMAS,SUMPO,SUMTO,SUMRVT,SUM_ENTPY,SUMTSTAT,SUMPSTAT) 
C
C******************************************************************************
C
      IF(IFEND.EQ.1)  THEN
...
C
      DO 5640  J=1,JM
      SUMAS(J) =  SUMAS(J) + SHRDFLOW(J)*NBLADE(J)
     &         +  SUMBLEED(J) - SUMCWL(J)
      RATIO    =  SUMAS(J)/SUMAS(1)
      WRITE(6,5630) J,SUMAS(J),RATIO,ETA_LOSS(J),SUMPO(J),SUMTO(J),
     &                SUMRVT(J)
...
C     FLOW WOULD NEED TO BE SUPPLIED IF THERE WAS NO LOSS IN THE INTERNAL 
C     COOLING PASSAGES.
C
...
C
      TS_EJECT  = TORELB(NCB)
     &          /(1.+.5*(GAGAS-1)*MACHCOOL(NCB)*MACHCOOL(NCB))
C
      IF(IFGAS.EQ.0) THEN
...
C
C
      WRITE(6,*)'BLADE COOLING PATCH NUMBER ',NCB,' SPECIFIED STAGN. PRE
     &SSURE = ',POCOOL,' STAGNATION PRESSURE AT HOLE EXIT = ',PO_AT_HOLE
C
  150 CONTINUE
C
      DO 160 NCW = 1,NCOOLW
...
C
  ...
  161 CONTINUE
C
C     PO_AT_HOLE IS THE STAGNATION PRESSURE AT WHICH THE COOLING FLOW
C     WOULD NEED TO BE SUPPLIED IF THERE WAS NO LOSS IN THE INTERNAL 
C     COOLING PASSAGES
C
      PS_EJECT  = 0.25*SUM_PSTAT/SUM_MAS
      TS_EJECT  = TORELW(NCW)/(1.+.5*(GA-1)*MACHCOOL(NCW)*MACHCOOL(NCW))
C
      IF(IFGAS.EQ.0) THEN
...
C
     ...
     &                  + SUM_MAS*(HOCOOL - HO2IS_AT_HOLE)
      ...
      END IF
C
      WRITE(6,*) 'WALL COOLING PATCH NUMBER ',NCW,'SPECIFIED STAGN. PRES
     &SURE = ',POCOOL,' STAGNATION PRESSURE AT HOLE EXIT = ', PO_AT_HOLE
C
  160 CONTINUE
C
C
...
C
      FLOW_OUT =   SUMAS(J1) +  CFLOW_STG
C
      IF(IFGAS.EQ.0) THEN
           WNET     = CP*(SUMAS(J1)*TO_IN - FLOW_OUT*TO_OUT) + ENTHIN
           TO2_IS   = TO_IN*(PO2/POIN)**FGA
           WMAIN_IS = SUMAS(J1)*CP*(TO_IN - TO2_IS)
      ELSE
     ...
     &                BALPHA1,BALPHA2)
           ...
           WMAIN_IS = SUMAS(J1)*(HO_IN - HO2_IS)
           WNET     = SUMAS(J1)*HO_IN  + ENTHIN - FLOW_OUT*HO_OUT
      END IF
C
      WTOT_IS  =   WMAIN_IS + WCOOL_IS
      PCCOOL   =   100.*CFLOW_STG/SUMAS(J1)
C
...
C
      WRITE(6,*) '*****************************************************'
      WRITE(6,*)
      WRITE(6,*) ' USING THE VALUES OF COOLANT STAGNATION PRESSURE'
      WRITE(6,*) ' INPUT AS DATA - WHICH MAY NOT BE COMPATIBLE WITH'
      WRITE(6,*) ' THE STATIC PRESSURE AND MACH NUMBER AT THE EJECTION'
      WRITE(6,*) ' POINT - THEN: '
      WRITE(6,*)
      WRITE(6,*) ' TOTAL TO TOTAL ISENTROPIC EFFICIENCY, ALLOWING FOR '
      WRITE(6,*) ' THE POTENTIAL WORK OF ALL COOLING FLOWS AND THE PUMPI
     &NG POWER = ',ETATT
      WRITE(6,*)
      WRITE(6,*) ' NEGLECTING THE PUMPING POWER, ETA-TT                 
     &         = ',ETATT_NOP
      WRITE(6,*)
      WRITE(6,*) '*****************************************************'
      WRITE(6,*)
      WRITE(6,*) ' ASSUMING NO PRESSURE LOSS IN THE COOLANT SUPPLY'
      WRITE(6,*) ' PASSAGES AND USING THE STATIC PRESSURE AT THE'
      WRITE(6,*) ' EJECTION HOLES TO FIND THE COOLANT SUPPLY STAGNATION'
      WRITE(6,*) ' PRESSURE - THEN:'
      WRITE(6,*)
      WRITE(6,*) ' TOTAL TO TOTAL ISENTROPIC EFFICIENCY, ALLOWING FOR '
      WRITE(6,*) ' THE POTENTIAL WORK OF ALL COOLING FLOWS AND THE PUMPI
     &NG POWER = ',ETATT_AT_HOLE
      WRITE(6,*)
      WRITE(6,*) ' NEGLECTING THE PUMPING POWER, ETA_TT                 
     &        =  ',ETA_ATH_NOP
      WRITE(6,*)
C
 3333 CONTINUE
...
C
                       REACTION = (SUMTSTAT(JMIX_STAGE)- SUMTSTAT(J2M1))
     &                 /(SUMTSTAT(J1)         - SUMTSTAT(J2M1))
                       IF(PORAT.GT.0.0) REACTION = 1.0 - REACTION
C     
      WRITE(6,*)  
...
C
      IF(IFGAS.EQ.0) THEN
           WORK = (CP*(SUMAS(J1)*TO_IN - SUMAS(J2M1)*TO_OUT))/1000.
      ELSE
           WORK = (SUMAS(J1)*HO_IN - SUMAS(J2M1)*HO_OUT)/1000.
      END IF
C
      WRITE(6,*)
...
C
      IF(IFGAS.EQ.0) THEN
           WORK = (  CP*(SUMAS(1)*TO_IN  - SUMAS(JMM1)*TO_OUT)
     &          +  ENTHIN  - HBLEDTOT  )/1000.
      ELSE
           WORK = (SUMAS(1)*HO_IN  - SUMAS(JMM1)*HO_OUT 
     &          + ENTHIN  - HBLEDTOT)/1000.
      END IF
C   
      WRITE(6,*)
      WRITE(6,*) ' INLET ENTHALPY FLUX,  KW  = ',CP*SUMAS(1)*TO_IN/1000.
      WRITE(6,*) ' OUTLET ENTHALPY FLUX, KW  = ',
     &             CP*SUMAS(JMM1)*TO_OUT/1000.
